...
# Firebase config
FIREBASE_URL = "https://king-65bab-default-rtdb.asia-southeast1.firebasedatabase.app"
FIREBASE_SECRET = os.environ.get("FIREBASE_SECRET", "dvO5XFT1qDviweMr0dtrc04fz7HEpkRYgR9E952b")

app = Flask(__name__)
app.secret_key = 'npchosting-super-secret-key-2026'
...
def load_users():
    try:
        r = requests.get(f"{FIREBASE_URL}/npchosting_users.json?auth={FIREBASE_SECRET}", timeout=5)
        data = r.json()
        if not data:
            default = {"Npc0": {"password": "Npc321", "role": "admin"}}
            save_users(default)
            return default
        if 'Npc0' not in data:
            data['Npc0'] = {"password": "Npc321", "role": "admin"}
            save_users(data)
        return data
    except Exception as e:
        print(f"Firebase load error: {e}")
        return {"Npc0": {"password": "Npc321", "role": "admin"}}

def save_users(data):
    try:
        requests.put(
            f"{FIREBASE_URL}/npchosting_users.json?auth={FIREBASE_SECRET}",
            json=data,
            timeout=5
        )
    except Exception as e:
...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# NpcHosting - Default Bot
import time

print("=" * 40)
print("Bot is running on NpcHosting")
...
def api_create_server():
    ...
    if not username:
        username = f"NPC_{random.randint(10000, 99999)}"
...
def login():
    if request.method == 'POST':
        ...
        users = load_users()
        if username == 'Npc0' and password == users.get('Npc0', {}).get('password'):
            session['user'] = 'admin'
            session['role'] = 'admin'
            return redirect(url_for('admin_dashboard'))
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 50)
    print("🚀 NpcHosting - Bot Hosting Panel")
    print("=" * 50)
    print(f"📍 Landing: http://localhost:{port}")
    print(f"📍 Admin: http://localhost:{port}/login")
    print(f"🔗 API: http://localhost:{port}/api/create")
    print("👤 Npc0 / Npc321")
    print("=" * 50 + "\n")
